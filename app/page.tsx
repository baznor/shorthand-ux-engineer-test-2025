    ...
    <div className="flex flex-col rounded-3xl shadow-lg max-w-[1000px] w-full mx-12 mt-4 mb-12 bg-card-background">
      <div className="grid grid-cols-1 lg:grid-cols-2">
        <form onSubmit={handleSubmit(onSubmit)} className="p-8 pt-4 flex flex-grow flex-col">
          ...
          <TextInput value={name} onChange={value => setName(value)} autofocus={true} label="Name *" icon="mdi:account" placeholder="Name *" type="name" name="name" id="name" register={register} rules={ {required: '* Required'} } errors={errors}/>
          <TextInput value={email} onChange={value => setEmail(value)} label="Email *" icon="mdi:at" placeholder="Email *" type="email" name="email" id="email" register={register} rules={ {required: '* Invalid email'} } errors={errors}/>
          <TextInput value={password} onChange={value => setPassword(value)} label="Password *" icon="mdi:password" placeholder="Password *" type="password" name="password" id="password" hint="* must be at least 8 characters" register={register} rules={ {required: '* must be at least 8 characters', minLength: { value: 8, message: "* must be at least 8 characters" }} } errors={errors}/>                             
